...
#ifndef __ROBUSTGRAPH_H__
#define __ROBUSTGRAPH_H__

#include <string.h>
#include "robustblock.h"
...
{
	if (node_num_max < 16) node_num_max = 256; // BAGON changed from 16
	if (edge_num_max < 16) edge_num_max = 256; // BAGON changed from 16
    /* BAGON changed malloc to new 
	nodes = (node*) malloc(node_num_max*sizeof(node));
	arcs = (arc*) malloc(2*edge_num_max*sizeof(arc)); */
    nodes = new node[node_num_max];
    arcs = new arc[2*edge_num_max];
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_nodes(int num)
{
    ...
    int old_node_num = node_num_max;
	node* nodes_old = nodes;

	node_num_max += node_num_max / 2;
	if (node_num_max < node_num + num) node_num_max = node_num + num;
    /* BAGON changed realloc to new 
	nodes = (node*) realloc(nodes_old, node_num_max*sizeof(node)); */    
    nodes = new node[node_num_max];
//     nodes = (node*) my_new_realloc(nodes_old, old_node_num*sizeof(node), node_num_max*sizeof(node));
    
	if (!nodes) { if (error_function) (*error_function)("Not enough memory!"); exit(1); }
    
    memcpy(nodes, nodes_old, old_node_num * sizeof(node)); // BAGON
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_arcs()
{
    ...
    arcs = new arc[arc_num_max];
//     arcs = (arc*) my_new_realloc(arcs_old, old_arc_num*sizeof(arc), arc_num_max*sizeof(arc));
    
	if (!arcs) { if (error_function) (*error_function)("Not enough memory!"); exit(1); }
    
    memcpy(arcs, arcs_old, old_arc_num * sizeof(arc));    // BAGON
